...
int Painter_DrawTextWithLimitationC(int x, int y, pchar  text, Color::E color, int limitX, int limitY, int limitWidth, int limitHeight)
{
    ...
    while (*text)
    {
        x = DrawCharWithLimitation(x, y, (uchar)*text, limitX, limitY, limitWidth, limitHeight);
        retValue += Font_GetLengthSymbol((uchar)*text);
        text++;
    }
    return retValue + 1;
...
void Painter_DrawBigTextInBuffer(int eX, int eY, int size, pchar  text, uint8 buffer[320][240])
{
    for (int x = 0; x < 320; x++)
    {
        for (int y = 0; y < 240; y++)
