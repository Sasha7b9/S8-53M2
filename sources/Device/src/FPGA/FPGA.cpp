...
//    BufferFPGA &buffer = ch.IsA() ? data.A : data.B;
    ...
    uint8 *dat = ds->DataBegin(ch);
    const uint8 *const begin = dat;
    const uint8 *const end = ds->DataEnd(ch);
    ...
    funcRead();         // Это лишнее чтение сделано потому, что первая считанная точка бракованная.
                        // Для компенсации этого эффекта лишняя точка получается уменьшением адреса на один в FPGA::Reader::CalculateAddressRead()
