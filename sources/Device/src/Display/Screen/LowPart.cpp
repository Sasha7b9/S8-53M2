...
void LowPart::Draw()
{
    ...
    DString("р\xa5%s", TBase::ToString(tbase)).Draw(x, y0);
    ...
    if (MODE_WORK_IS_DIRECT)
    {
        ...
        DString("с\xa5\x10%s", source[TRIG_SOURCE]).Draw(x, y1, ColorTrig());
    }
...
void LowPart::WriteTextVoltage(Chan ch, int x, int y)
{
    ...
    if (enable)
    {
        ...
        DString("%s\xa5%s\xa5%s", (ch == Chan::A) ? (LANG_RU ? "1к" : "1c") : (LANG_RU ? "2к" : "2c"),
