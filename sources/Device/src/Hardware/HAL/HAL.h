...
namespace HAL_FMC
{
    ...
    extern uint8 *const ADDR_RAM_DATA_END;          // Размер памяти для размещения данных - 1МБ - 2 * 75кБ
    extern uint8 *const ADDR_RAM_DISPLAY_BACK;      // Размер 75кБ
    extern uint8 *const ADDR_RAM_DISPLAY_BACK_END;
